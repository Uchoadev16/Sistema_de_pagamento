...
<body class="bg-gray-900 font-sans text-gray-200 min-h-screen flex flex-col">
    ...
    // Nome do usuário (substitua pelo valor real da sessão)
    $user_name = isset($_SESSION['user_name']) ? htmlspecialchars($_SESSION['user_name']) : 'Usuário';
    // Caminho da foto do usuário (substitua pelo caminho real)
    $user_photo = '../assets/images/user_profile.jpg';
    ?>
    <header class="bg-black text-yellow-400 shadow-lg">
        <nav class="container mx-auto px-4 py-4 flex justify-between items-center">
            <div class="text-2xl font-bold">Banco Ouro</div>
            <div class="flex items-center space-x-4">
                <img src="<?php echo $user_photo; ?>" alt="Foto de perfil de <?php echo $user_name; ?>" class="w-10 h-10 rounded-full object-cover border-2 border-yellow-400">
                <span class="text-sm sm:text-base font-medium"><?php echo $user_name; ?></span>
                <a href="../controllers/logout.php" class="bg-yellow-500 text-gray-900 px-3 py-2 rounded-lg hover:bg-yellow-400 transition duration-300 text-sm font-semibold">Sair</a>
            </div>
        </nav>
    </header>
