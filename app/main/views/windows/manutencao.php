...
<head>
    ...
    <title>Sistema em Manutenção - Banco Ouro</title>
...
<body class="bg-gray-900 font-sans text-gray-200 min-h-screen flex flex-col">
    ...
    <main class="container mx-auto px-4 py-8 flex-grow flex items-center justify-center">
        <section class="text-center max-w-md">
            <i class="fas fa-tools text-yellow-400 text-5xl mb-4"></i>
            <h1 class="text-3xl sm:text-4xl font-bold text-yellow-400 mb-4">Sistema em Manutenção</h1>
            <?php session_start(); if (isset($_SESSION['message'])): ?>
                <div class="mb-4 p-3 rounded bg-yellow-600 text-gray-900 text-sm">
                    <?php echo htmlspecialchars($_SESSION['message']['text']); ?>
                    <?php unset($_SESSION['message']); ?>
                </div>
            <?php else: ?>
                <p class="text-base sm:text-lg text-gray-400 mb-6">Nosso sistema está em manutenção para melhorias. Volte em breve!</p>
            <?php endif; ?>
